...
file::file(){
	...
	if((block_info.inputs.size() != 0) && (block_info.outputs.size() != 0)){
		...
		file_ptr = fopen(block_info.key_value["file_name"], "a");
	} else if(block_info.outputs.size() != 0){
		//This block will read from a file
		file_type = FILE_READ;
		file_ptr = fopen(block_info.key_value["file_name"], "r");
		//Allocate a scratchspace for file data
		temp_read_data = (void*)(new int[FILE_READ_SIZE/sizeof(int)]);
